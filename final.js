...
var arraySugar= [];
var arraySalt = [];
for(var i=0;i<arrayCountry.length;i++)  // initialise array
{
	arraySugar[i]=0;
	arraySalt[i]=0;
...
lineReader.on('line',function(line)
{
    ...
    if(isHeader==0)           // fetching index of column
    {
    ...
    else
    {
        ...
        if(northEurope.includes(lineRecords[countryIndex])) // fetching northEurope countries data
        {
    ...
    if(centralEurope.includes(lineRecords[countryIndex]))  // fetching centralEurope countries data
    {
    ...
    if(southEurope.includes(lineRecords[countryIndex]))   // fetching southEurope countries data
    {
...
{ 
     ...
     fs.writeFile('json/countries1.json', JSON.stringify(arrayJson) , 'utf-8');
     var Europe=[];
